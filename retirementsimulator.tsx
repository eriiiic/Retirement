...
const RetirementSimulator = () => {
  ...
  const calculateSimulation = useCallback(() => {
    ...
    for (let year = 0; year <= simulationDuration; year++) {
      ...
      // Fast path: If capital is already zero, add zero data points but continue to generate data points
      // all the way to the target age for x-axis display
      if (capital <= 0) {
  ...
  useEffect(() => {
    setGraphData(calculatedData as GraphDataPoint[]);
  }, [calculatedData]);
            ...
            <div className="flex">
              ...
              <div>
                <h3 className="text-sm font-semibold text-red-800 mb-2">
                  {withdrawalMode === "amount" 
                    ? `Your Funds Will Last Until Age ${statistics.exhaustionAge}` 
                    : "Adjustment Needed"}
                </h3>
                <p className="text-xs text-red-700">
                  {withdrawalMode === "amount"
                    ? `With your current withdrawal amount of ${formatAmount(monthlyRetirementWithdrawal)}/month, your capital will be depleted after ${Math.round(statistics.exhaustionAge - statistics.retirementStartAge)} years of retirement.`
                    : `Your capital will be depleted by age ${statistics.exhaustionAge}, before reaching your target age of ${statistics.lifeExpectancy}. Consider increasing your savings, adjusting your retirement age, or reducing your planned withdrawals.`}
                </p>
              </div>
            </div>
          </div>
              ...
              <p className="text-sm">
                {withdrawalMode === 'amount'
                  ? (statistics.capitalAtRetirement >= statistics.totalNeededCapital 
                      ...
                      : `With a monthly withdrawal of ${formatAmount(monthlyRetirementWithdrawal)}, your funds will last ${Math.round(statistics.exhaustionAge - statistics.retirementStartAge)} years until age ${statistics.exhaustionAge}. Your retirement capital of ${formatAmount(statistics.capitalAtRetirement)} will be fully depleted during your retirement.`)
                ...
                <div className="mt-3 bg-white rounded-lg p-3">
                  ...
                  <ul className="text-xs text-gray-700 space-y-2">
                    <li className="flex items-start">
                      ...
                      <span>
                        Increase your monthly investment by{' '}
                        {formatAmount(
                          Math.ceil(
                            (statistics.totalNeededCapital - statistics.capitalAtRetirement) /
                            (12 * (statistics.calculatedRetirementStartYear - new Date().getFullYear()))
                          )
                        )}
                      </span>
                    </li>
